...
export const Grid1 = ({ blogs }: { blogs: Blog[] }) => {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-6 xl:gap-10 p-4 md:p-8">
      {blogs.map(({ id, title, image, category, slug, createdAt, user }) => {
        return (
          <div key={id} className="flex rounded-xl border overflow-hidden">
            ...
            <div className="flex flex-col justify-between p-6 bg-accent min-h-72 md: w-full">
              ...
              <span className="truncate text-sm">
                ...
                >
                  {user?.name} <ArrowUpRight size={12} />
                </Link>
              </span>
            </div>
          </div>
